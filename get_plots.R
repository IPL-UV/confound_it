...
DD <- reshape2::melt(allres, measure.vars = nm[c(1:8)],
                     variable.name = "stats")
DD <- DD[DD$size > 10 & DD$dist %in% c("gaussian", "exponential"),]
...
ggplot() + 
  geom_hline(mapping = aes(yintercept = y), data = data.frame(stats = "AER", y = 1), linetype = 4) +
  geom_boxplot(data = DD[DD$stats  %in% c("AbsCor", "AER", "AE"),], aes(y = value, x = size, colour = method, fill = method, group = groups)) +
  #geom_line() + 
...
ggsave("plot_error_causal_coeff_boxplots.pdf", width = 5, height = 4, units = "in")
...
ggplot() + 
  ...
  geom_line(mapping = aes(x = size, y = value.median, colour = method, linetype = method), 
            data = DDM[DDM$stats %in% c("AbsCor", "AE", "AER"),],
            linewidth = 1) + 
  #geom_ribbon(aes(x = size, ymin = value.min, ymax = value.max, colour = NULL), alpha = 0.3) + 
  scale_x_log10(breaks = sizes, labels = sizes) + scale_y_log10() + 
  ...
  #scale_y_continuous(limits = c(0,100)) +
  ylab("")  + 
  scale_linetype_manual(values = c("oracle" = "dotdash", "ICA-PCF" = "solid",  "PCA-PCF" = "dotted", "PLS-PCF" = "dashed")) + 
  scale_color_manual(values = c("oracle" = "black", "ICA-PCF" = "blue",  "PCA-PCF" = "red",
                                "PLS-PCF" = "purple")) + 
  facet_grid(cols = vars(dist), rows = vars(stats), scales = "free") + theme_bw() + 
